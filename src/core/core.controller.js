 ...
 */
function getValue(label, dataArray) {	
	var isHorizontal = me._chartInstance.type === 'horizontalBarChart';
...
function dataInLabel(dataArray, labelArray) {
	for (var i = 0; i < dataArray.length; i++) {
		...
		if (labelArray.indexOf(item.x) < 0) {
			return false;
		}
	}
	return true;
	...
	/** @lends Chart */ {
		...
		update: function(config) {
			...
			me._layers.sort(function(a, b) {
				return a.z === b.z ? a._idx - b._idx : a.z - b.z;
			});
		...
		_formatDataset: function(dataArray) {
			...
			var result = dataArray.slice(0);
			var match = dataInLabel(dataArray, labels);
			var isHorizontal = me._chartInstance.type === 'horizontalBarChart';
			if (match && dataArray.length < labels.length) {
				for (var i = 0; i < labels.length; i++) {
					var label = labels[i];
					if(isHorizontal) {
						result[i] = {y: label, x: getValue(label, dataArray)};
					} else {
						result[i] = {x: label, y: getValue(label, dataArray)};
					}
				}
			}
			return result;
